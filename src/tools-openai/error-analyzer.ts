...
export async function analyzeSteps(
    ...
    try {
        ...
        let json: ErrorAnalysisResponse = { recap: '', blame: '', improvement: '' }
        try {
            const content = (completion?.choices[0]?.message?.content as string).replace(/[\t\n]/g, '')
            json = JSON.parse(content) as ErrorAnalysisResponse
        } catch (e) {
            console.log(`🐹🐹🐹 analyzeSteps`, e)
            console.log(`🐹🐹🐹 analyzeSteps completion`, completion?.choices)
        }
