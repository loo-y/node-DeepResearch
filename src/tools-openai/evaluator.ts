...
export async function evaluateAnswer(
    ...
    try {
        ...
        let json: EvaluationResponse = { is_definitive: false, reasoning: '' }
        try {
            const content = (completion?.choices[0]?.message?.content as string).replace(/[\t\n]/g, '')
            json = JSON.parse(content) as EvaluationResponse
        } catch (e) {
            console.log(`🐹🐹🐹 evaluateAnswer`, e)
            console.log(`🐹🐹🐹 evaluateAnswer completion`, completion?.choices)
        }
