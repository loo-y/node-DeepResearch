...
export async function dedupQueries(
    ...
    try {
        ...
        let json: DedupResponse = { unique_queries: [], think: '' }
        try {
            const content = (completion?.choices[0]?.message?.content as string).replace(/[\t\n\s]/g, '')
            json = JSON.parse(content) as DedupResponse
        } catch (e) {
            console.log(`🐹🐹🐹 dedupQueries`, e)
            console.log(`🐹🐹🐹 dedupQueries completion`, completion?.choices)
        }
