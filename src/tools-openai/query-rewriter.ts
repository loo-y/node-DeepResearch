...
export async function rewriteQuery(
    ...
    try {
        ...
        let json: KeywordsResponse = { queries: [], think: '' }
        try {
            const content = (completion?.choices[0]?.message?.content as string).replace(/[\t\n]/g, '')
            json = JSON.parse(content) as KeywordsResponse
        } catch (e) {
            console.log(`🐹🐹🐹 rewriteQuery`, e)
            console.log(`🐹🐹🐹 rewriteQuery completion`, completion?.choices)
        }
