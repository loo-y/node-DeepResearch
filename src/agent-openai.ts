...
import { rewriteQuery } from './tools-openai/query-rewriter'
...
export async function getResponse(
    ...
    while (context.tokenTracker.getTotalUsage() < tokenBudget && badAttempts <= maxBadAttempts) {
        ...
        try {
            const content = (completion?.choices[0]?.message?.content as string).replace(/[\t\n]/g, '')
            thisStep = JSON.parse(content)
        } catch (e) {
            console.log(`🐹🐹🐹 getResponse`, e)
            console.log(`🐹🐹🐹 getResponse completion`, completion?.choices)
        }
    ...
    if (isAnswered) {
        ...
        try {
            ...
            console.log(`🐹🐹🐹 beast mode`, e)
            console.log(`🐹🐹🐹 beast mode completion`, completion?.choices)
        }
        // const model = genAI.getGenerativeModel({
...
export async function main() {
    ...
    const { result: finalStep, context: tracker } = (await getResponse(question)) as {
        result: AnswerAction
